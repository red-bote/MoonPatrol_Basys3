#!/bin/bash
# Author the Basys3 top level for Moon Patrol and place it where
# moonpatrol_basys3.xpr expects it.
#
# Unlike the Dar-convention machines (which adapt an existing pristine
# DE10-lite top level and record the change as a diff patch), this port has
# no equivalent VHDL top to diff against: Arcade-MoonPatrol.sv/
# moonpatrol_mist.sv are MiSTer/MiST board glue this port replaces entirely,
# not adapts. moonpatrol_basys3.vhd is therefore original, authored directly
# against rtl/target_top.vhd's interface -- see contrib/basys3/PORTING_SPEC.md.
#
# Requires `make setup` to have run first (applies the dpram Xilinx-BRAM
# patch and generates tools/roms/prom_moonpatrol.vhd).
...
TARGET_SRC="$PROJ_DIR/moonpatrol_basys3.srcs/sources_1/new"
TARGET="$TARGET_SRC/moonpatrol_basys3.vhd"
